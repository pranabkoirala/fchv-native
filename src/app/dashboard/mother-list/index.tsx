        ...
        <View className="px-5">
          {loading ? (
            ...
            filtered?.map((mother) => {
              ...
              const wks = mother.lmp
                ? Math.floor(
                    (new Date().getTime() - lmpTime) /
                      (1000 * 60 * 60 * 24 * 7),
                  )
                : 0;
                ...
                >
                  <View className="flex-row justify-between items-start mb-4">
                    <View className="flex-1 pr-4">
                      ...
                      <Text className="text-gray-400 font-bold text-xs mt-1">
                        Age: {mother.age || "26"} •{" "}
                        {mother.pregnancy_count || "1"}st Pregnancy
                      </Text>
                    </View>
                    <View
                  ...
                  <View className="flex-row gap-3 mb-6">
                    <View className="flex-1 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                      <Text className="text-gray-400 font-black text-[10px] uppercase">
                        LMP
                      </Text>
                  ...
                  <View className="flex-row justify-between items-center mb-4">
                    ...
                    <Text className="text-blue-600 font-black text-[13px]">
                      {wks < 40
                        ? `Next Visit: Week ${[12, 16, 20, 24, 28, 32, 36, 40].find((m) => m > wks)}`
                        : "Completed"}
                    </Text>
                  </View>
                  ...
                  <View className="flex-row items-center justify-between px-1">
                    {[1, 2, 3, 4, 5, 6, 7, 8].map((v, i) => {
                      const completed =
                        wks >= [12, 16, 20, 24, 28, 32, 36, 40][i];
                      const current =
                        !completed &&
                        (i === 0 ||
                          wks >= [12, 16, 20, 24, 28, 32, 36, 40][i - 1]);
                        ...
                        <View key={v} className="flex-row items-center flex-1">
                          ...
                          >
                            {completed ? (
                              ...
                              <Text
                                className={`text-[11px] font-black ${current ? "text-primary" : "text-gray-400"}`}
                              >
                                {v}
                              </Text>
                            )}
                          ...
                          {i < 7 && (
                            <View
                              className={`h-[2px] flex-1 mx-1 ${completed ? "bg-[#006F62]" : "bg-gray-100"}`}
                            />
                          )}
                        </View>
                      );
