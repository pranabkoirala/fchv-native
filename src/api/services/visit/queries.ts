...
import {
  insertToTempVisitTable,
  moveTempToRealVisitTable,
...
const fetchVisitsFromServer = async (
  ...
  let URL = `${API_LIST.visits.get}?page=1&page_size=200`;
  let isEOR = false;

  await clearTable("visit_staging");
  ...
  while (!isEOR) {
    ...
    if (results.length) {
      await insertToTempVisitTable(results);
    }
  }

  await moveTempToRealVisitTable();
  await clearTable("visit_staging");
};

export { fetchVisitsFromServer };
