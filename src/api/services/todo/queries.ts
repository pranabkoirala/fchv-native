...
import {
  insertToTempTodoTable,
  moveTempToRealTodoTable,
...
const fetchTodosFromServer = async (
  ...
  let URL = `${API_LIST.todo.get}?page=1&page_size=200`;
  let isEOR = false;

  await clearTable("todo_staging");
  ...
  while (!isEOR) {
    ...
    if (results.length) {
      await insertToTempTodoTable(results);
    }
  }

  await moveTempToRealTodoTable();
  await clearTable("todo_staging");
};

export { fetchTodosFromServer };
